...
class Freezeit {
private:
    const char* LOG_PATH = "/sdcard/Android/Frozenlog";
...
public:
    ...
    Freezeit(int argc) {
        ...
        auto fp = fopen("/data/adb/modules/Frozen/module.prop", "r");
        if (!fp) {
            fprintf(stderr, "找不到模块属性文件: /data/adb/modules/Frozen/module.prop");
            exit(-1);
        }
    ...
    bool saveProp() {
        auto fp = fopen("/data/adb/modules/Frozen/module.prop", "wb");
        if (!fp)
            return false;
